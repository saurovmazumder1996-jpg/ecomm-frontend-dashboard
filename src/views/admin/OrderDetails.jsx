import React from "react";

function OrderDetails() {
  return (
    <div className="px-2 lg:px-7 pt-5 ">
      <div className="w-full p-4 bg-[#1e293b] rounded-md">
        <div className="flex justify-between items-center p-4">
          <h2 className="text-xl text-[#d0d2d6]">Order Details</h2>
          <select
            name=""
            id=""
            className="px-4 py-2 focus:border-green-500 outline-node bg-[#475569] border border-green-700 rounded-md text-[#d0d2d6]"
          >
            <option value="">Pending</option>
            <option value="">Processing</option>
            <option value="">Warehouse</option>
            <option value="">Placed</option>
            <option value="">Cancel</option>
          </select>
        </div>

        <div className="p-4 ">
          <div className="flex gap-2 text-lg text-[#d0d2d6]">
            <h2>#456781</h2>
            <span>18 July 2025</span>
          </div>
          <div className="flex flex-wrap">
            <div className="w-[32%]">
              <div className="pr-3 text-[#d0d2d6] text-lg">
                <div className="flex flex-col gap-1">
                  <h2 className="pb-2 font-semibold">
                    Deliver To : Pranab Gandhi
                  </h2>
                  <p>
                    <span className="text-sm">
                      2930 Pearl Street Suite 100, Boulder, CO 80301-1124, USA
                    </span>
                  </p>
                </div>
                <div className="flex justify-start items-center gap-3">
                  <h2>Payment Status:</h2>
                  <span className="text-base">Paid</span>
                </div>
                <span>Price: Rs.2390</span>
                <div className="mt-4 flex flex-col gap-4 bg-green-700 rounded-lg ">
                  <div className="text-[#d0d2d6]">
                    <div className="flex gap-3 text-md">
                      <img
                        className="w-[55px] h-[80-px]"
                        src="http://localhost:3000/images/category/1.jpg"
                        alt=""
                      />
                      <div className="text-white">
                        <h2>Product Name</h2>
                        <p>
                          <span>Brand: </span>
                          <span>Adidas </span>
                          <span className="text-lg ">Quantity: 4</span>
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="mt-4 flex flex-col gap-4 bg-green-700 rounded-lg ">
                  <div className="text-[#d0d2d6]">
                    <div className="flex gap-3 text-md">
                      <img
                        className="w-[55px] h-[80-px]"
                        src="http://localhost:3000/images/category/1.jpg"
                        alt=""
                      />
                      <div className="text-white">
                        <h2>Product Name</h2>
                        <p>
                          <span>Brand: </span>
                          <span>Adidas </span>
                          <span className="text-lg ">Quantity: 4</span>
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="mt-4 flex flex-col gap-4 bg-green-700 rounded-lg ">
                  <div className="text-[#d0d2d6]">
                    <div className="flex gap-3 text-md">
                      <img
                        className="w-[55px] h-[80-px]"
                        src="http://localhost:3000/images/category/1.jpg"
                        alt=""
                      />
                      <div className="text-white">
                        <h2>Product Name</h2>
                        <p>
                          <span>Brand: </span>
                          <span>Adidas </span>
                          <span className="text-lg ">Quantity: 4</span>
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>

            <div className="w-[68%] ">
              <div className="pl-3">
                <div className="mt-4 flex flex-col bg-green-700 rounded-md p-4">
                  <div className="text-[#d0d2d6] ">
                    <div className="flex justify-start items-center gap-3">
                      <h2>Seller 1 Order: </h2>
                      <span>Pending</span>
                    </div>
                  </div>
                  <div className="flex gap-3 text-md mt-2">
                    <img
                      className="w-[55px] h-[80-px]"
                      src="http://localhost:3000/images/category/1.jpg"
                      alt=""
                    />
                    <div className="text-white">
                      <h2>Product Name</h2>
                      <p>
                        <span>Brand: </span>
                        <span>Adidas </span>
                        <span className="text-lg ">Quantity: 4</span>
                      </p>
                    </div>
                  </div>

                  <div className="text-[#d0d2d6] mt-2">
                    <div className="flex justify-start items-center gap-3">
                      <h2>Seller 1 Order: </h2>
                      <span>Pending</span>
                    </div>
                  </div>
                  <div className="flex gap-3 text-md mt-2">
                    <img
                      className="w-[55px] h-[80-px]"
                      src="http://localhost:3000/images/category/1.jpg"
                      alt=""
                    />
                    <div className="text-white">
                      <h2>Product Name</h2>
                      <p>
                        <span>Brand: </span>
                        <span>Adidas </span>
                        <span className="text-lg ">Quantity: 4</span>
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

export default OrderDetails;
